...
- New **Astra** preset (`id: astra`, Glow category): luminous crisp-glow look — exposure −5, brilliance −15, and brightness −5 merge onto the engine's single `brightness` slider as −25, with highlights −30, contrast +15, vibrance +15, warmth −5 (`temperature`), tint +5, sharpness +35, and definition +10 (`clarity`) creating the glow. Registered at full default strength so the recipe applies verbatim, with matching curve/split-tone/HSL shaping, adaptive tuning, and scene affinity for the recommender. Covered by `src/test/astra-preset.test.ts`.
